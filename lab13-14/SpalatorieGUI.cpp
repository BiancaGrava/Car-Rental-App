...
void RetetaGUI::initGUI()
{
	...
	formMeds->addRow("Nr INMATRICULARE:", this->txtDenumire1);
	formMeds->addRow("PRODUCATOR:", this->txtProducator1);
	formMeds->addRow("MODEL:", this->txtPret1);
	formMeds->addRow("TIP:", this->txtSubstantaActiva1);

	auto formReteta = new QFormLayout;
	formReteta->addRow("NR INMATRICULARE:", this->txtDenumire2);
	formReteta->addRow("PRODUCATOR:", this->txtProducator2);
	formReteta->addRow("MODEL:", this->txtPret2);
	formReteta->addRow("TIP:", this->txtSubstantaActiva2);
...
void RetetaGUI::initConnect()
{
	...
	QObject::connect(this->butonExportHTML, &QPushButton::clicked, [&]()
		{
			this->s.exportHTML("spalatorie.html");
			QMessageBox::information(nullptr, "Export", "Datele au fost exportate in fisierul 'spalatorie.html'");
		});

	QObject::connect(this->butonExportCSV, &QPushButton::clicked, [&]()
		{
			this->s.exportCSV("spalatorie.csv");
			QMessageBox::information(nullptr, "Export", "Datele au fost exportate in fisierul 'spalatorie.csv'");
		});
	...
	QObject::connect(this->butonAdauga, &QToolButton::clicked, [&]()
		{
			try
			{
				...
				auto pret = this->txtPret1->text().toStdString();
				auto substantaActiva = this->txtSubstantaActiva1->text().toStdString();
	...
	QObject::connect(this->lst1, &QListWidget::itemSelectionChanged, [&]()
		{
			...
			else
			{
				...
				this->txtPret1->setText(QString::fromStdString(med.getModel()));
				this->txtSubstantaActiva1->setText(QString::fromStdString(med.getTip()));
			}
		});
	...
	QObject::connect(this->lst2, &QListWidget::itemSelectionChanged, [&]()
		{
			...
			else
			{
				...
				this->txtPret2->setText(QString::fromStdString(med.getModel()));
				this->txtSubstantaActiva2->setText(QString::fromStdString(med.getTip()));
			}
		});
}
