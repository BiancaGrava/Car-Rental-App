...
void Service::addMed(const string& denumire, const string& producator, const string& pret, const string& substantaActiva)
{
...
void Service::modifyMed(const string& denumire, const string& producator, const string& pret, const string& substantaActiva, const int& camp)
{
	double probabilitate = (rand() % 100) * 1.0 / 100;
	if (probabilitate < repo.getErrorPercentage())
	{
	...
	try
	{
		...
		if (camp == 1)
		{
			const Medicament& m{ denumire, producator, pret, "automata" };
			val.validate(m);
			repo.modify(m, camp);
		}
		...
		if (camp == 2)
		{
			const Medicament& m{ denumire, producator, "nou", substantaActiva };
			val.validate(m);
			repo.modify(m, camp);
		}
...
void Service::deleteMed(const string& denumire, const string& producator)
{
	...
	const Medicament& m{ denumire, producator, "nou", "automata"};
	val.validate(m);
	repo.del(m);
...
vector<Medicament> Service::filterMeds(const string& pret, const string& substantaActiva, const int& criteriu) const
{
	vector<Medicament> rez;
	vector<Medicament> meds = repo.getMeds();
	if (criteriu == 1)
		copy_if(meds.begin(), meds.end(), back_inserter(rez), [=](const Medicament& m) {return m.getModel() < pret; });
	if (criteriu == 2)
		copy_if(meds.begin(), meds.end(), back_inserter(rez), [=](const Medicament& m) {return m.getTip() < substantaActiva; });
	return rez;
}
...
/*double Service::stats() const
{
	...
	for (auto it : vect)
		rez.push_back(it.getProducator());
...
void Service::addReteta(const string& denumire, const string& producator, const string& pret, const string& substantaActiva)
{
	Medicament m{ denumire, producator, pret, substantaActiva };
	val.validate(m);
	ret.addReteta(m);
...
void Service::random(int nrMeds)
{
	...
	{
		string denumire = getMeds()[i].getInmatriculare();
		string producator = randomString();
		string substantaActiva = randomString();
		string pret = randomString();
