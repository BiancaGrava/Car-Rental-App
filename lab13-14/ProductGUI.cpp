...
void ProductGUI::styleForm()
{
	...
	QPixmap pixmap("car.jpeg");
	QIcon icon(pixmap);
	this->setWindowIcon(icon);
	this->setWindowTitle("Spalatorie");
	...
	QObject::connect(this->butonAdauga, &QPushButton::clicked, [&]()
		{
			...
			try
			{
				this->s.addMed(denumire.toLocal8Bit().constData(), producator.toLocal8Bit().constData(), pret.toLocal8Bit().constData(), substantaActiva.toLocal8Bit().constData());
				loadData(this->s.getMeds(), "#2e2eb8");
				adaugaButoane(this->s.getMeds());
			}
			catch (ValidateException&)
			{
				QMessageBox::warning(nullptr, "Date invalide", "Masina nevalida");
			}
			catch (RepositoryException&)
			{
				QMessageBox::warning(nullptr, "Date invalide", "Masina deja exista!");
			}
		});
	...
	QObject::connect(this->butonSterge, &QPushButton::clicked, [&]()
		{
			...
			{
				QMessageBox::warning(nullptr, "Date nevalide", "Masina nevalida");
			}
			catch (RepositoryException&)
			{
				QMessageBox::warning(nullptr, "Date nevalide", "Masina nu exista");
			}
		});
	...
	QObject::connect(this->butonActualizeaza, &QPushButton::clicked, [&]()
		{
			...
			try
			{
				this->s.modifyMed(denumire.toLocal8Bit().constData(), producator.toLocal8Bit().constData(), pret.toLocal8Bit().constData(), substantaActiva.toLocal8Bit().constData(), 3);
				loadData(this->s.getMeds(), "#2e2eb8");
				adaugaButoane(this->s.getMeds());
			}
			catch (ValidateException&)
			...
			{
				QMessageBox::warning(nullptr, "Date invalide", "Masina nu exista");
			}
		});
	...
	QObject::connect(this->butonCauta, &QPushButton::clicked, [&]()
		{
			...
			try
			{
				...
				string str = "Masina are modelul " + med.getModel() + " si tipul " + med.getTip();
				QMessageBox::information(nullptr, "Info",QString::fromStdString(str));
			}
			catch (RepositoryException&)
			{
				QMessageBox::warning(nullptr, "Oof!", "Masina cautata nu exista!");
			}
		});
	...
	QObject::connect(this->butonFiltreaza, &QPushButton::clicked, [&]()
		{
			...
			{
				meds = this->s.filterMeds(pret.toStdString(), substantaActiva.toStdString(), 1);
			}
			if (pret == nullptr && substantaActiva != nullptr)
			{
				meds = this->s.filterMeds(pret.toStdString(), substantaActiva.toStdString(), 2);
			}
			if (pret != nullptr && substantaActiva != nullptr)
			{
				meds = this->s.filterMeds(pret.toStdString(), substantaActiva.toStdString(), 1);
			...
			if (meds.size() == 0)
			{
				if (this->txtPret->text() == "" && this->txtSubstantaActiva->text() == "")
					QMessageBox::warning(nullptr, "Atentie", "Va rog completati cu valori in casuta de model sau de tip");
				else
					QMessageBox::information(nullptr, "Filtrare nereusita", "Nu s-au gasit masini care sa satisfaca criteriile!");
			}
			else
			{
	...
	QObject::connect(this->butonSorteazaDenumire, &QPushButton::clicked, [&]()
		{
			...
			if (meds.size() == 0)
			{
				QMessageBox::information(nullptr, "Sortare nereusita", "Nu s-au gasit masini pentru sortare!");
			}
			else
			{
				loadData(meds, "#2e4e99");
	...
	QObject::connect(this->butonAdaugaReteta, &QToolButton::clicked, [&]()
		{
			try
			{
				...
				auto pret = this->txtPret->text().toStdString();
				auto substantaActiva = this->txtSubstantaActiva->text().toStdString();

				this->s.addReteta(denumire, producator, pret, substantaActiva);
			}
			catch (RepositoryException&)
			{
				QMessageBox::warning(nullptr, "Atentie", "Masina inexistenta");
			}
		});
	...
	QObject::connect(this->lst, &QListWidget::itemSelectionChanged, [&]()
		{
			...
			else
			{
				...
				this->txtPret->setText(QString::fromStdString(med.getModel()));
				this->txtSubstantaActiva->setText(QString::fromStdString(med.getTip()));
			}
		});
}
...
void ProductGUI::loadData(const vector<Medicament>& meds, const QColor& color)
{
	...
	{
		QListWidgetItem* medicament = new QListWidgetItem(QString::fromStdString(med.getInmatriculare()));
...
void ProductGUI::initGUI()
{
	...
	formLy->addRow("NR INMATRICULARE:", this->txtDenumire);
	formLy->addRow("PRODUCATOR:", this->txtProducator);
	formLy->addRow("MODEL:", this->txtPret);
	formLy->addRow("TIP:", this->txtSubstantaActiva);
...
void ProductGUI::adaugaButoane(const vector<Medicament>& meds)
{
	...
	for (auto producator : producatori)
	{
		...
		QObject::connect(btn, &QPushButton::clicked, [=]()
			{
				string str = "Masini produse de: " + producator + " : " + to_string(howMany);
